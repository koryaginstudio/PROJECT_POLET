...
export function App() {
  ...
  const pinRoute = (id: string | null) => {
    /* Щелчок по пути, когда выбрана его заявка, показывает сам путь, а не
       снимает выбор.

       Это был тупик: щёлкнув по точке, диспетчер получал её маршрут на
       карте — и повторный щелчок по этому же маршруту попадал в «тот же,
       значит снять». Ответ о маршруте оказывался недостижим: до него
       нельзя было дойти от заявки, только снять всё и начать заново. */
    const fromOrder = selection.kind === 'order' && id !== null;
    setPinnedRoute((prev) => (prev === id && !fromOrder ? null : id));
    if (id) setRouteFocus((n) => n + 1);
    /* Выбор маршрута снимает выбранную заявку. Спрашивают об одном за раз, а
       сводка отвечает сперва о заявке: щёлкнув по точке, а потом по пути,
       диспетчер видел, как подсветка перешла на маршрут, а карточка внизу
       осталась от заявки — и читалось это как несработавший щелчок. */
    if (id) setSelection(OVERVIEW);
  };
