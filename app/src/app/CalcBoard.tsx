...
interface Props {
  ...
  onEdit: () => void;
  /** Запущен ли движок. Правка по событию — его работа, и без него кнопка
      гаснет с подсказкой, почему. */
  engineLive: boolean;
    ...
    <section className="panel calc" aria-label="Расчёт">
      <div className="calc__head">
        ...
        <div className="calc__intro">
          <h2 className="calc__run" title="Открытый расчёт">
            {run}
          </h2>
        ...
        <div className="calc__actions">
          <span className="calc__group">
            ...
            <span title="Пересобрать этот день с другими настройками. Прошлый расчёт останется в архиве, на другие расчёты правка не влияет">
              <Button
                variant="primary"
                size="sm"
                onClick={onManual}
            ...
            {/* Подсказка висит на обёртке: кнопка системы своего title не
                принимает, а объяснить, что это за правка, надо. Без движка
                кнопка погашена, и подсказка говорит почему: серая кнопка без
                объяснения читается как поломка. */}
            <span
              title={
                engineLive
                  ? 'День пошёл не так: авария, инженер выбыл, инженер задержится. Программа расчёта пересоберёт остаток смены за полторы секунды'
                  : 'Правка по событию недоступна: программа расчёта не запущена. Как включить — в настройках, на вкладке «Данные»'
              }
            >
              <Button
                variant="secondary"
          ...
          <span className="calc__group">
            ...
            <span title="Книга Excel: сводка, маршруты, заявки по порядку объезда, заявки без инженера и штат">
              <Button
                variant="ghost"
                size="sm"
                onClick={onExport}
