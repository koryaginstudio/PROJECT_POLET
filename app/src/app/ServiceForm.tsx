...
/* Полный список настроек сервиса: границы смены и пороги, по которым интерфейс
   ...
   движок про них не знает, и расчёт от них не меняется ни на визит. */
...
const LIMITS: { title: string; note: string; rows: Limit[] }[] = [
  ...
  },
  {
    ...
    note: 'Точка у числа «Разрыв загрузки»: во сколько раз самый занятый инженер загружен сильнее самого свободного',
    rows: [
    ...
    <>
      <section className="panel">
        ...
        <p className="clients__lede">
          Это не настройки движка. Движок отвечает на вопрос «как раскладывать», и его переменные
          меняют план. Здесь другое: границы рабочего дня и пороги, по которым интерфейс называет
          число проблемой. Расчёт от них не меняется ни на визит — меняется то, что мы считаем
          нормой. Хранится в браузере: это настройка рабочего места, движок про неё не знает.
        </p>
        {patched > 0 && (
      ...
      <section className="panel">
        ...
        <p className="clients__lede">
          На эти границы встают шкала времени, канбан, воронка этапов и признак «окно уже открыто».
          Заводские 07:00–21:00 — из контракта стенда; смена с ночными бригадами в них не
          укладывается. Если день в открытом расчёте кончается позже, шкала сама дотянется до
          его конца: у данных заказчика это 22:00.
        </p>
        ...
        <div className="srvform__hint">
          ...
          <span>
            ...
            опечатка. Данные при этом остаются как есть: если движок прислал визит за пределами
