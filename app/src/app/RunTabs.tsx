...
/* Что означают три числа через косую черту. Порядок тот же, что и в самой
   строке: сколько работы, сколько рук, что вышло. */
const META_HINT =
  'Заявок в расчёте / инженеров на маршрутах / прогноз выполнения';
        ...
        {shown.map((run, index) => (
          ...
          <button
            ...
            title={
              full && !isOn(run.id)
                ? fullHint
                : `${run.code} · ${whenLabel(run.created)} · заявок ${run.ordersTotal}` +
                  ` · инженеров на маршрутах ${run.engineersOnShift}` +
                  ` · прогноз выполнения ${dec(run.coverage)} %`
            }
          >
            <span className="runtabs__code">{run.code}</span>
            {/* Три числа рядом с номером: заявок, инженеров, прогноз. Номер
                сам по себе ничего не говорит — расчёты в ленте различают не
                по нему, а по тому, чем они кончились, и раньше за этим
                приходилось открывать каждый.

                Числа идут через косую черту, без подписей при каждом. В ленте
                их полтора десятка подряд, и подписи «з · инж · %» повторялись
                у каждого расчёта, отнимая место у самих чисел: строка
                читалась как сплошной текст, а не как три величины, которые
                сравнивают по столбцам. Что означает каждое, говорит подсказка
                на самих числах, а не сокращение при них. */}
            <span className="runtabs__meta" title={META_HINT}>
