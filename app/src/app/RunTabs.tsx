        ...
        {shown.map((run, index) => (
          ...
          >
            ...
            {/* Три числа рядом с номером: заявок, инженеров, покрытие. Номер
                ...
                Числа подписаны, а не разделены дробью: «205/34/86%» надо
                было расшифровывать, а «205 з · 34 инж · 86 %» читается с
                первого раза. Полная расшифровка — во всплывающей подсказке
                на самой кнопке. */}
            <span className="runtabs__meta" aria-hidden="true">
              {run.ordersTotal} з · {run.engineersOnShift} инж · {Math.round(run.coverage)} %
            </span>
          </button>
          </Fragment>
        ))}
