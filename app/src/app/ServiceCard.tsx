    ...
    >
      ...
      {!dense && crew && staff !== undefined && (
        <div className={'srvcrew' + (crew.able.length <= staff / 3 ? ' srvcrew--few' : '')}>
          ...
          {/* Второй строкой — имена, пока их мало. Дальше только счёт: про
              то, кому чего не хватает, разговаривает карточка записи, где
              есть место сказать это по-человечески. Здесь такая оговорка
              занимала бы строку на каждой из семнадцати карточек. */}
            ...
            {crew.able.length === 0
              ...
              : crew.able.length <= 8
                ...
                : crew.able.length >= staff * 0.75
                  ? 'почти весь штат'
                  : `остальные ${staff - crew.able.length} взять не могут`}
          </span>
        </div>
      )}
