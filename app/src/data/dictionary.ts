...
/* ─── состояние инженера на день ──────────────────────────────────────────

   Три значения контракта 1.2. Не путать со статусом визита: тот про заявку и
   меняется в течение дня, этот — про человека и отвечает на единственный
   вопрос, выходит он сегодня или нет. «Сегодня не выйдет» — это тот самый
   сценарий перепланирования, которого требует ТЗ. */

const CREW_STATUS_NAMES: Record<string, string> = {
  on_shift: 'В смене',
  off_shift: 'Выходной',
