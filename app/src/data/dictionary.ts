...
/* Два исхода из семи закрывают заявку: по выполненной ехать некуда, по
   отменённой — незачем. В выгрузке за прошедший день таких больше половины,
   и раскладывать их по маршрутам значило бы планировать уже сделанную
   работу. Правило живёт здесь, а не в планировщике: закрытую заявку обязаны
   одинаково узнавать и расчёт, и сводка, и канбан. */
const CLOSED_STATUSES = new Set(['done', 'cancelled']);

/** Закрыта ли заявка до расчёта. Пустой статус — открыта: движок 1.1 статусов
    не отдаёт, и все его заявки в работе. */
export const orderClosed = (order: { status?: string | null }) =>
  CLOSED_STATUSES.has(order.status ?? '');

/* ─── статус инженера ────────────────────────────────────────────────────── */

/* Три значения контракта. Вышел, не вышел, выбыл: последнее — сценарий
   перепланирования из ТЗ, и от «не на смене» оно отличается тем, что
   случилось внезапно. */
const ENGINEER_STATUS_NAMES: Record<string, string> = {
  on_shift: 'На смене',
  off_shift: 'Не на смене',
  unavailable: 'Выбыл'
};

export const engineerStatusName = (key: string) => ENGINEER_STATUS_NAMES[key] ?? key;

/** Вышел ли инженер сегодня. Пустой статус — вышел: движок 1.1 статусов не
    знает, и весь его штат на смене. То же правило, по которому планировщик
    отбирает, кому раздавать заявки. */
export const engineerOnShift = (engineer: { status?: string | null }) =>
