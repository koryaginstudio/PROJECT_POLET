...
export interface EngineRun {
  ...
  forms?: string[];
  /** Спрятана ли запись из списка. Черновик остаётся в архиве и
      открывается по своему номеру — из списка уходит только строка.
      Необязательное: движок старой сборки поля не присылает. */
  скрыт?: boolean;
  /** Чем запись посчитана, если не сегодняшним движком. Движок сверяет
      отпечаток своего кода с тем, что помнит запись; совпал — поля нет.
      Приходит и в списке расчётов, и у отдельной записи (контракт 1.2,
      «что изменилось в 1.5»).

      `словами` — готовая фраза движка, и показывать надо её, а не свою:
      устаревает разное. У сохранённого пересчёта (`план_с_диска: true`)
      план хранится файлом и показан таким, каким его сохранили, — устарел
      сам план. У обычного расчёта план пересчитывается при открытии
      сегодняшним кодом, а устарели числа `summary` на карточке. Одна
      фраза на оба случая была бы неправдой в одном из них. */
  движок?: {
    /** Отпечаток движка, которым считали. `null` — запись старше того дня,
        когда движок стал помнить свой отпечаток. */
    тогда: string | null;
    сейчас: string;
    план_с_диска: boolean;
    словами: string;
  };
...
/** Архив расчётов. Спрятанные черновики в список не приходят — за ними
    `listRuns(true)`; движок отдаёт их с признаком `скрыт` у каждого.

    `скрытых` — сколько записей спрятано, чтобы интерфейсу было что
    показать строкой «спрятано N»: сами записи для этого просить не надо. */
export const listRuns = (withHidden = false) =>
  call<{ runs: EngineRun[]; скрытых?: number }>(withHidden ? '/runs?hidden=1' : '/runs');

/** Спрятать запись из списка или вернуть её обратно.

    Это не удаление, и слова «удалить» здесь быть не должно: запись лежит
    в архиве, открывается по своему номеру, её план и журнал целы,
    сохранённое сравнение с ней читается. Уходит только строка из списка —
    и вместе с ней двойной счёт в справочниках, которые из этого списка и
    собраны. Удаления у движка нет намеренно: номер записи он выдаёт как
    «длина архива плюс один», и вырванная середина отдала бы новому
    расчёту занятый номер. */
export const hideRun = (id: string, скрыт: boolean) =>
  call<EngineRun & { скрытых: number }>(`/runs/${encodeURIComponent(id)}/hidden`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ скрыт })
  });
...
/** Убрать выгрузку. Движок откажет (409), если по ней есть расчёты.
    ...
    `убрано_из_кэша` — сколько производных файлов ушло вместе с ней.
    Прежде они переживали удаление, и день висел в `ready` призраком,
    дающим 400 по нажатию (контракт 1.2). Поле необязательное: движок
    старой сборки его не присылает. */
export const deleteUpload = (day: string) =>
  call<{ deleted: string; убрано_из_кэша?: number }>(
    `/uploads/${encodeURIComponent(day)}`,
    { method: 'DELETE' }
  );
