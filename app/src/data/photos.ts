...
/** Лицо одного инженера. Считается из табельного номера, поэтому одинаково в
    ...
    Нумерация табельных начинается то с единицы (`E001` в файлах вёрстки),
    то с нуля (`E00` у движка). Нулевой номер давал индекс −1 и оставался без
    лица: в базе он один такой и стоял со знаком среди сорока одной
    фотографии. Ему достаётся последний снимок — тот, который при нумерации с
    нуля не может достаться никому другому: последний номер такого штата на
    единицу меньше длины списка. */
export function faceOf(engineer: Faced): string {
  ...
  const index = seat === 0 ? PHOTOS.length - 1 : seat - 1;
