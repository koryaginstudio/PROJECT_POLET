...
/* Календарь словами. Лежит рядом с часами нарочно: всё, чем интерфейс
   называет время, собрано в одном месте — иначе «четверг» в мониторинге и
   «четверг» в его меню разъезжаются по написанию. */
const WEEKDAYS = [
  'воскресенье',
  'понедельник',
  'вторник',
...
/** День недели словом: «четверг». */
export const weekdayName = (date: Date) => WEEKDAYS[date.getDay()];

/** Месяц в родительном падеже: «сентября» — для «25 сентября». */
export const monthName = (date: Date) => MONTHS[date.getMonth()];

/** Число и месяц цифрами: «25.09». Год не пишем — смотрят на сегодняшний
    день, и год в нём лишний знак. */
export const dayDot = (date: Date) =>
