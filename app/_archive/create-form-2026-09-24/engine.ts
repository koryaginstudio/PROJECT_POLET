...
export const RULE_TITLES: Record<EngineRule, { label: string; what: string }> = {
  churn_penalty: {
    ...
    what: 'Насколько расчёт сохраняет объявленный план при пересчёте. Ноль — план перестраивается свободно.'
  },
  buffer_base: {
    label: 'Зазор до первой заявки, мин',
    what: 'Запас перед первой заявкой маршрута. Как правило, не требуется.'
  }
};
...
export const CHURN_PRESETS: ChurnPreset[] = [
  ...
  },
  {
    ...
    measured: '86–89% заявок сохраняют исполнителя'
  },
  {
    value: 'rebuild',
...
export function durationHint(factor: number): string {
  if (factor < 0.95) return 'Планирование плотнее оценки: заявок больше, риск опозданий выше.';
...
export function bufferHint(step: number): string {
  if (step === 0) return 'Зазор отсутствует: опоздание переносится на следующую заявку целиком.';
  if (step <= 4) return 'Небольшой зазор: маршрут плотный, отклонения компенсируются слабо.';
  if (step <= 10) return `Умеренный зазор: к пятой заявке — ${Math.round(step * Math.sqrt(5))} мин.`;
  return `Осторожный маршрут: к пятой заявке — ${Math.round(step * Math.sqrt(5))} мин, заявок в день меньше.`;
}
