...
# Допуск упрощения ломаной, метры. OSRM отдаёт сотни точек на поездку через
# город — это верно до сантиметра и бессмысленно на экране: линия рисуется в
# три с половиной пикселя. Двадцать метров — тот же допуск, с которым работает
# движок, и под него написана раскладка маршрутов по полосам в интерфейсе.
SIMPLIFY_METERS = 20.0

# Перегон в Каширу — девяносто километров, и на нём двадцать метров допуска
# дают ломаную в тысячу точек, из которых на экране города неразличимы все.
# Поэтому допуск растёт с длиной: на дальнем перегоне важна трасса, а не
# съезды с неё.
SIMPLIFY_PER_KM = 2.0
