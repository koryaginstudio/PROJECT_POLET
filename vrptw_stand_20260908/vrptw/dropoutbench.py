...
from .replan import (apply_dropout, carve_remainder, plan_as_is,
                     simulate_until, stability)
...
def _ci(values: list[float]) -> tuple[float, float]:
    """Среднее и половина 95% интервала."""
    if len(values) < 2:
    ...
    for r in range(RUNS):
        ...
        def replan(rest, anc):
            ...
            p = fast.solve(rest, params=improved.Params(
                ...
                anchor=anc, churn_penalty=CHURN_PENALTY), seed=seed)
            assert not p.check_invariants(rest)
        ...
        a_total, a_his = played(whole, plan_as_is(whole, st))     # A
        b_total, b_his = played(whole, plan_b)                    # B
        c_total, c_his = played(maimed, plan_as_is(maimed, st))   # C
...
def _report(back_after: int | None):
    ...
    print("Инженер выбыл: жать пересчёт или ехать как ехали?"
          "  (визитов за день, парно по дням, 95%)\n")
