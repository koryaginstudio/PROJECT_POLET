...
from .replan import (carve_remainder, plan_as_is, simulate_until, stability,
                     urgent_orders)
...
def _one_day(args) -> dict:
    ...
    for r in range(RUNS):
        ...
        out = simulate_once(rest, plan_as_is(rest, st), rest_rng())
        ...
        for d in DELAYS:
            ...
            plan_d = fast.solve(rest_d, params=improved.Params(
                lns_seconds=secs,
                anchor=anchor, churn_penalty=CHURN_PENALTY), seed=seed)
            if d == 0:
                acc["solve_seconds"].append(time.time() - t0)
            assert not plan_d.check_invariants(rest_d)
...
def main():
    ...
    print(f"\nпересчёт против его отсутствия: аварий +{avg('d0_urgent'):.2f}, "
          f"всего визитов {avg('d0_all') - avg('as_is_all'):+.1f}")
    print(f"сразу против часа промедления:  аварий "
          f"{avg('d0_urgent') - avg('d60_urgent'):+.2f}, всего визитов "
          f"{avg('d0_all') - avg('d60_all'):+.1f}")
...
def _churn_day(args) -> dict:
    ...
    for r in range(10):
        ...
        assert not new_plan.check_invariants(rest)
