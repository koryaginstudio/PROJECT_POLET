...
В момент показа интернет не нужен вовсе, всё берётся из кэша.
    ...
    python3 -m vrptw.core.osm check     что лежит в кэше и похоже ли на правду
"""
...
def fetch_matrix(addresses: list[dict]) -> tuple[np.ndarray, np.ndarray]:
...
class OSMNetwork:
    ...
    def __init__(self, address_file: Path | None = None,
        ...
        free_flow = blob["duration"].astype(np.float32)
        self.road_km = blob["distance"].astype(np.float32)
        n = len(self.nodes)
    ...
    def travel_minutes(self, from_node: int, to_node: int,
                       depart_minute: int) -> float:
        if from_node == to_node:
            return LAST_MILE
        hour = max(min(HOURS), min(max(HOURS), depart_minute // 60))
        return float(self._matrices[hour][from_node, to_node]) + LAST_MILE
...
if __name__ == "__main__":
    ...
    if what == "fetch":
        ...
        print(f"не знаю режима {what!r}: есть fetch, warm и check")
