...
def _rush(table: dict[int, float], hour: int) -> float:
    return table.get(max(7, min(20, hour)), 1.0)
...
class RoadNetwork:
    ...
    def travel_minutes(self, from_node: int, to_node: int, depart_minute: int) -> float:
        """
        ...
        if from_node == to_node:
            return 3.0
        hour = max(7, min(20, depart_minute // 60))
        return float(self._matrices[hour][from_node, to_node]) + 3.0
