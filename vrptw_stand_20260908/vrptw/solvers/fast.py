...
from ..core.domain import Day, Engineer, Order, Plan, Visit
from .base import make_route
from .improved import (COMFORT_SLACK, COOLING, RUIN_GEO, RUIN_SEGMENT,
                       SINGLE_OPTION, START_TEMPERATURE, Params)
...
class _Route:
    ...
    def _step(self, clock: int, node: int, order: Order, position: int):
        ...
        travel = int(round(self.net.travel_minutes(node, order.node, clock)))
        arrive = clock + travel
...
class _Solution:
    def __init__(self, day: Day, params: Params):
        ...
        self.capable: dict[str, list[str]] = {
            o.id: [e.id for e in day.engineers if e.can_do(o)] for o in day.orders
        }
    ...
    def objective(self) -> tuple[int, float]:
        return (len(self.pool), sum(r.cost for r in self.routes.values()))
    ...
    def ruin(self, rng: random.Random, q: int) -> None:
        ...
        if roll < RUIN_GEO:
            ...
            def slack(pair):
                ...
                travel = int(round(self.net.travel_minutes(node, o.node, clock)))
...
def solve(day: Day, params: Params | None = None, seed: int = 0,
    ...
    while (iterations < budget) if budget is not None else (time.time() < deadline):
        ...
        if obj < cur_obj:
            take = True
        elif obj[0] == cur_obj[0]:
            delta = obj[1] - cur_obj[1]
