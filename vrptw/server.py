...
def отпечаток_кода() -> str:
    ...
    **И кэш адресов.** Координата заявки — такие же данные плана, как
    строка выгрузки, а отпечаток её не видел: 22-го правка двух адресов
    востока в `geocode.json` оставила бы в кэше и в готовых план со
    старыми точками. Сети отдельно не нужны: их файлы названы
    отпечатком набора точек, и новые координаты — это новая сеть. Кэш
    адресов пишется только загрузчиком зоны, и только когда адреса в нём
    нет; выгрузки диспетчера ищут адреса в свой файл состояния.
    """
    ...
    файлы.append(Path(geocode.CACHE_FILE))
...
def _проверка_отпечатка_адресов() -> tuple[int, int]:
    """Координата — данные плана: правка кэша адресов меняет отпечаток.

    Иначе кэш и готовые отдают план со старыми точками — так было бы
    22-го с двумя адресами востока. Две копии кэша рядом, одинаковые во
    всём, кроме широты одного адреса: имя файла тоже идёт в хэш, и сверка
    с настоящим файлом увидела бы разницу путей, а не содержимого."""
    import tempfile
    from .core import geocode as гк

    прежний = гк.CACHE_FILE
    try:
        адреса = json.loads(прежний.read_text(encoding="utf-8"))
        какой = next(a for a in sorted(адреса) if адреса[a].get("lat"))
        отпечатки = []
        for сдвиг in (0.0, 0.001):
            путь = Path(tempfile.mkdtemp(prefix="vrptw-check-адреса-")) / "geocode.json"
            путь.write_text(json.dumps(
                {**адреса, какой: {**адреса[какой], "lat": адреса[какой]["lat"] + сдвиг}},
                ensure_ascii=False, sort_keys=True), encoding="utf-8")
            гк.CACHE_FILE = путь
            отпечатки.append(отпечаток_кода.__wrapped__())
    except Exception as exc:  # noqa: BLE001
        print(f"  ✗ отпечаток и кэш адресов — {type(exc).__name__}: {exc}")
        return 0, 1
    finally:
        гк.CACHE_FILE = прежний
    if отпечатки[0] != отпечатки[1]:
        print(f"  ✓ {'правка координаты меняет отпечаток':<38}"
              f"{отпечатки[0]} → {отпечатки[1]}")
        return 1, 0
