...
from ..core.domain import Day, Engineer, Order, Plan, require_valid
from .base import fits_stock, make_route, schedule, schedule_upto
...
# Насколько двигать вперёд заявку с обязательством на сегодня, минут
# сожаления на единицу веса. Та же шкала, что у аварий (`priority * 25`),
# чтобы две срочности были сравнимы между собой.
SLA_REGRET = 25.0
...
# не меняется ни при каком весе.
BALANCE_WEIGHT = 8.0

# Во сколько раз сорванное обязательство дороже переноса. Подобрано
# замером (`slabench`) на двух расписаниях смен и проверено на днях
# 13–30: −2,5 срыва в день при покрытии в пределах шума. Дальше двух
# срывов почти не убавляется, а покрытие начинает проседать.
SLA_WEIGHT = 2.0

# Приоритет распределения, названный организаторами: «Авария →
# Подключение → Ремонт / Дозаказ. Авария имеет максимальный приоритет».
# Вес несделанной заявки по ступени приоритета (0 — ремонт и дозаказ,
# 1 — подключение, 2 — авария).
#
# До этого приоритет в цель не входил вовсе: первый уровень был просто
# числом несделанного, и солверу было всё равно, что оставить лежать —
# аварию или ремонт; короткие работы ему даже выгоднее, их больше
# влезает. На востоке так лежало 1,7 аварии из 8 при назначенном
# ремонте, на юго-востоке — 9 подключений.
#
# Веса строго лексикографические в пределах дня: одна авария дороже
# 99 подключений, одно подключение дороже 99 ремонтов. Строгость
# держится, пока в дне меньше ста заявок одной ступени; на днях крупнее
# (`scalebench` доходит до 600) порядок становится весовым, а не строгим.
PRIORITY_WEIGHTS = (1.0, 100.0, 10000.0)

# Цена часа задержки аварийной заявки, в единицах стоимости маршрута
# (та же шкала, что у минут дороги).
#
# Постановщик на сессии вопросов 16 сентября: «как можно раньше нужно
# выполнить, да. Это авария», потому что обязательство перед абонентом —
# закрыть аварию за 24 часа. В данных у таких заявок окно стоит на целые
# сутки (08:00–22:00), и без этого члена модель понимает широкое окно как
# свободу: аварии вставали в план в среднем к 15:45, а пять из одиннадцати
# на Юго-востоке — после 16:00.
#
# Ноль возвращает поведение, на котором сняты все прежние числа.
URGENCY_WEIGHT = 0.0

# Вес среднего уровня цели — числа задействованных исполнителей.
#
# ТЗ называет две обязательные метрики, и первая из них — «выполнение
# всех заявок наименьшим количеством персонала». До сих пор солвер
# минимизировал только несделанное и стоимость маршрутов, то есть
# оптимизировал не то, по чему нас будут сравнивать.
#
# Единица означает строгий лексикографический порядок: план на девяти
# инженерах лучше плана на десяти при любой разнице в километрах.
# Ноль убирает уровень целиком и возвращает прежнее поведение в
# точности — это нужно `optimumbench`, где MILP максимизирует просто
# число визитов и наш планировщик обязан решать ту же задачу, а не свою.
VEHICLE_WEIGHT = 1.0

# Оба значения стоят значениями по умолчанию нарочно. Раньше они были
# нулевыми, а продакшен подставлял их вручную — и получалось, что замеры
# меряют один планировщик, а на демо едет другой. Ровно та же ошибка,
# что была с двумя реализациями солвера и с бюджетом в секундах. Теперь
# `Params()` без аргументов — это то, что работает у заказчика, а замер,
# которому нужно иначе, говорит об этом явно.
...
class Params:
    ...
    # Во сколько раз сорванное обязательство дороже переноса.
    #
    # Половина дня — заявки, которые нельзя отложить на завтра: срок по
    # SLA истекает раньше конца рабочего дня. Остальные можно. До сих пор
    # солвер не различал их вовсе: он максимизировал число визитов, а
    # бизнес считает нарушенные обещания абоненту. Единица — прежнее
    # поведение, всё равноценно.
    #
    # Вес работает в двух местах: в целевой функции несделанное
    # обязательство стоит во столько раз дороже, и при сборке такая
    # заявка двигается вперёд так же, как авария, — пропорционально весу.
    sla_weight: float = SLA_WEIGHT
    urgency_weight: float = URGENCY_WEIGHT

    # Вес несделанного по ступени приоритета, см. PRIORITY_WEIGHTS.
    # (1.0, 1.0, 1.0) — прежнее поведение: несделанное просто считается.
    # Так его пришпиливает `optimumbench`: точная модель максимизирует
    # число визитов, и сравнивать с ней надо на той же задаче.
    priority_weights: tuple = PRIORITY_WEIGHTS

    # Насколько дорог лишний задействованный инженер. 1.0 — строго
    # лексикографически, 0.0 — уровень выключен, см. VEHICLE_WEIGHT.
    ...
    # Закреплено диспетчером: заявка -> инженер. В отличие от `anchor`
    # это не штраф, а запрет: человек сказал «поедет этот», и торговать
    # его решением за минуту экономии солвер не вправе. Закреплённая
    # заявка либо у названного инженера, либо не назначена вовсе —
    # третьего диспетчеру предлагать нечестно.
    #
    # Проверяется там же, где навык: «кто вообще может взять эту
    # заявку» — одно понятие, и закрепление его сужает.
    pinned: dict | None = None

    # Что у исполнителей в машине: инженер -> прибор -> штук. Тоже запрет,
    # а не штраф: заявку с роутером не отдать тому, у кого роутера нет.
    # `None` — не ограничено, так строится утренний план: выдача под него
    # и делается. Инженер, которого в словаре нет, везёт пустую машину.
    # См. `base.fits_stock` — одна проверка на обе реализации.
...
class _State:
    ...
    def route_cost(self, visits, eng_id: str) -> float:
        ...
        # на устранение. Платим за каждый час от начала окна до начала
        # работ. В `fast` тот же член начисляется повизитно — сумма та же.
        if p.urgency_weight:
            поздно = sum(max(0, v.start - v.order.window_start)
                         for v in visits if v.order.priority >= 2)
            cost += p.urgency_weight * поздно / 60.0
    ...
    def unplaced_cost(self) -> float:
        ...
        """
        w = self.params.sla_weight
        pw = self.params.priority_weights
        if w == 1.0 and pw == (1.0, 1.0, 1.0):
            return float(len(self.pool))
        # Приоритет распределения: несделанная авария стоит дороже любого
        # числа несделанного ниже неё. Зеркально с `fast.unplaced_cost`.
        return sum(pw[o.priority] * (w if o.must_today else 1.0)
                   for o in self.pool)

    def objective(self) -> tuple[float, float, float]:
        """
        Минимизируем лексикографически, в порядке, который задаёт ТЗ:

            1. несделанное — «решение должно стремиться распределить все
               заявки»;
            2. число задействованных исполнителей — первая обязательная
               метрика;
            3. стоимость маршрутов — время в пути плюс подобранные
               штрафы; вторая обязательная метрика, пробег, ему прямо
               пропорциональна.

        Сравнение кортежей и есть лексикографический порядок, ничего
        дополнительно взвешивать не нужно.
        """
        cost = 0.0
        used = 0
        for eng_id in self.seq:
            ...
            if v is None:
                return (10_000.0, 0.0, 0.0)   # невалидное состояние
            if self.seq[eng_id]:
                used += 1
            cost += self.route_cost(v, eng_id)
        return (self.unplaced_cost(),
                self.params.vehicle_weight * used,
                cost)
    ...
    def best_insertion(self, order: Order, skip_eng: set[str] | None = None):
        ...
        for eng_id, seq in self.seq.items():
            ...
            if not eng.can_do(order):
                continue
            # Закрепление диспетчером — такой же запрет, как навык.
            # Зеркально с `_Solution.capable` в fast.py.
            if self.params.pinned:
                за_кем = self.params.pinned.get(order.id)
                if за_кем is not None and за_кем != eng_id:
                    continue
            # Оборудование в машине — такой же запрет. Зеркально с
            # `_Solution.options_for` в fast.py.
            if self.params.stock is not None and not fits_stock(
                    seq, order, self.params.stock.get(eng_id, {})):
                continue
    ...
    def recreate(self, rng: random.Random | None = None) -> None:
        ...
        while self.pool:
            ...
            for order in self.pool:
                ...
                if self.params.sla_weight != 1.0 and order.must_today:
                    regret += (self.params.sla_weight - 1.0) * SLA_REGRET
    ...
    def to_plan(self) -> Plan:
        ...
        return Plan(routes=routes, unassigned=list(self.pool), pinned=self.params.pinned,
                    duration_factor=self.params.duration_factor,
...
def solve(day: Day, params: Params | None = None, seed: int = 0,
    ...
    while (iterations < budget) if budget is not None else (time.time() < deadline):
        ...
        if obj < cur_obj:
            ...
            # Отжиг разрешает временно ухудшить стоимость — но только
            # внутри своего лексикографического уровня. Пустить его
            # через число людей значило бы, что средний уровень не
            # соблюдается вовсе.
    ...
    # Как и в `fast.py`: план проверяет себя сам.
